...
TOKEN* SCANNER::Scan() {
    ...
    while ((c = Fd->GetChar()) != EOF) {
        ...
        switch (c) {
            case '+': return new TOKEN{lx_plus};
            case '-': return new TOKEN{lx_minus};
            case '*': return new TOKEN{lx_star};
            case '/': return new TOKEN{lx_slash};
            case '=': return new TOKEN{lx_eq};
            case '!':
                if (Fd->GetChar() == '=') return new TOKEN{lx_neq};
                break;
            case '<':
                if (Fd->GetChar() == '=') return new TOKEN{lx_le};
                return new TOKEN{lx_lt};
            case '>':
                if (Fd->GetChar() == '=') return new TOKEN{lx_ge};
                return new TOKEN{lx_gt};
            case ':':
                if (Fd->GetChar() == '=') return new TOKEN{lx_colon_eq};
                return new TOKEN{lx_colon};
            case '.': return new TOKEN{lx_dot};
            case ';': return new TOKEN{lx_semicolon};
            case ',': return new TOKEN{lx_comma};
            case '(': return new TOKEN{lx_lparen};
            case ')': return new TOKEN{lx_rparen};
            case '[': return new TOKEN{lx_lbracket};
            case ']': return new TOKEN{lx_rbracket};
            case '{': return new TOKEN{lx_lbracket};
            case '}': return new TOKEN{lx_rbracket};
            default:
